-- ============================================================================
-- BaseCommand Core Tables — Run in Supabase SQL Editor
-- Safe to re-run: uses IF NOT EXISTS for all objects
-- Note: subscriptions and ai_usage tables already exist from prior migrations
-- ============================================================================
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own accounts"
    ON renewal_accounts FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_accounts_user ON renewal_accounts(user_id);
CREATE INDEX IF NOT EXISTS idx_accounts_renewal_date ON renewal_accounts(renewal_date);
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own context items"
    ON context_items FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_context_account ON context_items(account_id);
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own threads"
    ON conversation_threads FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_threads_account ON conversation_threads(account_id);
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own messages"
    ON conversation_messages FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own tasks"
    ON task_items FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_tasks_user ON task_items(user_id);
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own autopilot actions"
    ON autopilot_actions FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
CREATE INDEX IF NOT EXISTS idx_autopilot_user ON autopilot_actions(user_id);
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own kb docs"
    ON kb_documents FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own cache"
    ON analysis_cache FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
...
DO $$ BEGIN
  CREATE POLICY "Users can only access their own metrics"
    ON renewal_metrics FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
EXCEPTION WHEN duplicate_object THEN NULL;
END $$;
