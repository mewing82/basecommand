...
import {
  ...
  RefreshCw, Compass, MessageSquare, BookOpen, BarChart3,
  Radio, Send, LineChart, Upload, Image, Type, Trash2, Eye,
} from "lucide-react";
...
const renewalStore = {
  ...
  addMessage(threadId, message) {
    ...
    // Update parent thread's lastMessageAt
    return message;
  },
...
function CommandPalette({ onClose, setView, decisions, tasks, priorities }) {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      if (q.startsWith("/")) {
        const actions = [
          { type: "action", label: "Intel", cmd: "/intel", action: () => { setView("intel"); onClose(); } },
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(340px, 1fr))", gap: 14 }}>
          {filtered.map(d => (
            <DecisionCard
              ...
              onNavigateToIngest={(sessionId) => { setFocusSessionId(sessionId); setView("intel"); }}
            />
          ))}
        </div>
      )}
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: FONT_SANS, fontSize: 13, fontWeight: 600, color: C.amber, marginBottom: 2 }}>
                    {sourceSession.mode === "meeting" ? `Meeting: ${sourceSession.meetingTitle || "Untitled"}` : sourceSession.mode === "respond_to" ? "Response Draft" : "Extract Session"}
                  </div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, flex: 1, minWidth: 0 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
              <span style={{ fontFamily: FONT_MONO, fontSize: 11, color: C.amber, background: `${C.amber}18`, padding: "1px 6px", borderRadius: 4, textTransform: "uppercase", letterSpacing: "0.05em", fontWeight: 600 }}>Extract</span>
              <span style={{
...
// ─── Priorities View ──────────────────────────────────────────────────────────
function PrioritiesView({ priorities, setPriorities, decisions, tasks, projects }) {
  const [showForm, setShowForm] = useState(false);
  const [expandedId, setExpandedId] = useState(null);
  const [filterProject, setFilterProject] = useState(null);
  const [filterSource, setFilterSource] = useState(null);
...
// ─── Intel View (Command Chat + Extract) ─────────────────────────────────────
function IntelView({ setDecisions, setTasks, setPriorities, setMeetings, ingestSessions, setIngestSessions, focusSessionId, setFocusSessionId, decisions, tasks, priorities, projects }) {
  const [intelTab, setIntelTab] = useState(focusSessionId ? "extract" : "chat");
  const [hoveredTab, setHoveredTab] = useState(null);

  // Switch to extract tab when navigating from a decision's source link
  useEffect(() => {
    if (focusSessionId) setIntelTab("extract");
  }, [focusSessionId]);
  ...
  return (
    <div style={{ maxWidth: 760, margin: "0 auto", padding: "40px 40px 80px" }}>
      ...
      {/* Intel tab navigation */}
      <div style={{
        display: "flex", gap: 4, marginBottom: 24,
        borderBottom: `1px solid ${C.borderDefault}`, paddingBottom: 0,
      }}>
        {[
          ...
          return (
            <button
              ...
              style={{
                ...
                color: active ? C.textPrimary : hovered ? C.textPrimary : C.textSecondary,
                fontFamily: FONT_SANS, fontSize: 14, fontWeight: active ? 600 : 500,
                transition: "all 0.15s ease",
                marginBottom: -1,
              }}
            >
              <Icon size={16} strokeWidth={active ? 2 : 1.75} style={{ opacity: active ? 1 : 0.7 }} />
              {tab.label}
            </button>
          );
        })}
      ...
      {intelTab === "chat" && (
        <CommandChat decisions={decisions} tasks={tasks} priorities={priorities} projects={projects} />
      )}
      {intelTab === "extract" && (
        <ExtractView
          setDecisions={setDecisions} setTasks={setTasks} setPriorities={setPriorities} setMeetings={setMeetings}
          ingestSessions={ingestSessions} setIngestSessions={setIngestSessions}
          focusSessionId={focusSessionId} setFocusSessionId={setFocusSessionId}
        />
      )}
    </div>
  );
...
// ─── Command Chat ─────────────────────────────────────────────────────────────
const COMMAND_CHAT_CACHE_KEY = () => `bc2-${store._ws}-intel-chat-history`;

function CommandChat({ decisions, tasks, priorities, projects }) {
  const [messages, setMessages] = useState(() => safeParse(localStorage.getItem(COMMAND_CHAT_CACHE_KEY()), []));
  ...
  // Persist messages
  useEffect(() => {
    localStorage.setItem(COMMAND_CHAT_CACHE_KEY(), JSON.stringify(messages));
  }, [messages]);
  ...
  function buildSystemPrompt() {
    ...
    return `You are BC (Base Command), an executive decision intelligence co-pilot. You are in Command Chat — an open conversation with the user about their strategic work.

You have access to their current workspace state:
${JSON.stringify(snapshot)}

Today: ${new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}

GUIDELINES:
- Be direct, strategic, and specific. Reference their actual tasks, decisions, priorities by name.
- Help them think — don't just answer. Push back on fuzzy thinking, surface tradeoffs, connect dots.
- Format with markdown. Use **bold** for key points. Keep responses focused.
- You can help with: strategic thinking, decision analysis, priority review, meeting prep, communication drafting, problem solving, brainstorming.
- If they ask about something not in their workspace data, help them think it through anyway.`;
  }
  ...
  async function handleSend() {
    ...
    try {
      ...
      const errorMsg = { role: "assistant", content: `**Error:** ${err.message}`, timestamp: new Date().toISOString(), isError: true };
  ...
  function clearChat() {
    setMessages([]);
    localStorage.removeItem(COMMAND_CHAT_CACHE_KEY());
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 280px)", minHeight: 400 }}>
      ...
      <div style={{
        ...
        {messages.length === 0 && (
          <div style={{
            ...
            flex: 1, gap: 16, padding: "60px 0", textAlign: "center",
          }}>
            <div style={{
              width: 56, height: 56, borderRadius: 14,
              background: `linear-gradient(135deg, ${C.gold}20, ${C.aiBlue}20)`,
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              <Sparkles size={28} style={{ color: C.aiBlue }} />
            </div>
            <div>
              <h3 style={{ fontFamily: FONT_SANS, fontSize: 18, fontWeight: 600, color: C.textPrimary, margin: "0 0 6px" }}>
                Command Chat
              </h3>
              <p style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary, maxWidth: 400, lineHeight: 1.5, margin: 0 }}>
                Open conversation with BC. Ask about your priorities, think through decisions, prep for meetings, or brainstorm strategy.
              </p>
            </div>
        ...
        {messages.map((msg, i) => (
          <div key={i} style={{
            ...
            <div style={{
              ...
              background: msg.role === "user" ? C.goldMuted : C.aiBlueMuted,
              display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              {msg.role === "user"
                ? <span style={{ fontFamily: FONT_SANS, fontSize: 12, fontWeight: 700, color: C.gold }}>M</span>
            ...
            <div style={{
              ...
              background: msg.role === "user" ? C.bgElevated : C.bgAI,
              border: `1px solid ${msg.role === "user" ? C.borderSubtle : C.borderAI}`,
              ...(msg.isError ? { borderColor: C.red + "40" } : {}),
            }}>
              <div style={{
                fontFamily: FONT_BODY, fontSize: 14, color: C.textPrimary, lineHeight: 1.6,
                whiteSpace: "pre-wrap", wordBreak: "break-word",
              }}>
                {msg.content}
              </div>
              <div style={{
                fontFamily: FONT_MONO, fontSize: 10, color: C.textTertiary, marginTop: 6,
        ...
        {sending && (
          <div style={{ display: "flex", gap: 12 }}>
            <div style={{
              width: 32, height: 32, borderRadius: 8, flexShrink: 0,
              background: C.aiBlueMuted, display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              <Sparkles size={14} style={{ color: C.aiBlue }} />
            </div>
            <div style={{
              ...
              <div style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary, animation: "pulse 1.5s ease-in-out infinite" }}>
                Thinking...
              </div>
            </div>
          </div>
      ...
      <div style={{
        ...
        background: C.bgCard, border: `1px solid ${C.borderDefault}`, borderRadius: 12, padding: 8,
      }}>
        <textarea
          ref={inputRef}
          value={input}
          ...
          placeholder="Ask BC anything..."
          rows={1}
          style={{
            flex: 1, background: "transparent", border: "none", outline: "none",
            color: C.textPrimary, fontFamily: FONT_BODY, fontSize: 14,
            padding: "8px 12px", resize: "none", lineHeight: 1.5,
        ...
        <div style={{ display: "flex", gap: 4, alignItems: "center", padding: "0 4px 4px" }}>
          ...
          >
            <Send size={16} style={{ color: input.trim() && !sending ? "#fff" : C.textTertiary }} />
          </button>
        </div>
      </div>
    </div>
...
function ExtractView({ setDecisions, setTasks, setPriorities, setMeetings, ingestSessions, setIngestSessions, focusSessionId, setFocusSessionId }) {
  ...
  return (
    <div>
      {/* New / History toggle */}
      <div style={{ display: "flex", gap: 2, background: C.bgCard, border: `1px solid ${C.borderDefault}`, borderRadius: 8, padding: 3, marginBottom: 20, width: "fit-content" }}>
        {[
          { id: "new", label: "New" },
        ...
        meetings.length === 0 ? (
          <div style={{ textAlign: "center", padding: "80px 0" }}>
            ...
            <div style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.textTertiary, maxWidth: 340, margin: "0 auto" }}>
              Go to Intel → Extract, switch to <strong style={{ color: C.textSecondary }}>Meeting</strong> mode, and paste your notes. BC will summarize and extract action items.
            </div>
          </div>
        ) : (
...
function LibraryView({ documents, setDocuments, projects }) {
  const [expandedId, setExpandedId] = useState(null);
  const [search, setSearch] = useState("");
...
function RenewalsView() {
  ...
  function handleCreateAccount(data) {
    ...
    setAccounts(renewalStore.getAccounts());
  ...
  function handleDeleteAccount(id) {
    renewalStore.deleteAccount(id);
    setAccounts(renewalStore.getAccounts());
    if (selectedAccountId === id) setSelectedAccountId(null);
  }

  function handleUpdateAccount(id, updates) {
    const account = renewalStore.getAccount(id);
    if (!account) return;
    const updated = { ...account, ...updates, lastActivity: new Date().toISOString() };
    renewalStore.saveAccount(updated);
    setAccounts(renewalStore.getAccounts());
  }

  // Navigate to a specific account from Guidance
  function handleNavigateToAccount(accountId) {
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1200, margin: "0 auto" }}>
      {/* Header row: title + mode indicator */}
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontFamily: FONT_SANS, fontSize: 24, fontWeight: 700, color: C.textPrimary, margin: 0, letterSpacing: "-0.02em" }}>
            Renewal Operations
          </h1>
          <p style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary, margin: "4px 0 0" }}>
            AI-augmented renewal management
          </p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{
            ...
            <Sparkles size={14} style={{ color: C.aiBlue }} />
            <span style={{ fontFamily: FONT_MONO, fontSize: 12, fontWeight: 600, color: C.aiBlue, letterSpacing: "0.04em" }}>
              MODE: CO-PILOT
            </span>
          </div>
          <Btn variant="primary" onClick={() => setShowAddAccount(true)} size="sm">
      ...
      <div>
        {activeSection === "guidance" && (
          <RenewalsGuidance accounts={accounts} onNavigate={handleNavigateToAccount} />
        )}
        {activeSection === "accounts" && (
          <RenewalsAccountsView
            accounts={accounts}
        ...
        {activeSection === "forecast" && (
          <RenewalsPlaceholder
            ...
            description="Quarterly churn and retention forecast. Upload data, track WoW changes, generate executive briefs."
          />
        )}
        {activeSection === "knowledge" && (
          <RenewalsPlaceholder
            ...
            agent="Enablement Agent"
            description="Searchable repository of renewal documentation. Ask questions, get sourced answers."
          />
        )}
        {activeSection === "metrics" && (
          <RenewalsPlaceholder
            ...
            description="KPI tracking with AI interpretation. Upload data, get performance insights."
          />
        )}
      </div>
...
function AddAccountModal({ onClose, onCreate }) {
  ...
  function handleSubmit() {
    if (!name.trim() || !arr || !renewalDate) return;
    onCreate({ name, arr, renewalDate, riskLevel });
  }
  ...
  return (
    <Modal title="Add Account" onClose={onClose} width={480}>
      <FormField label="Account Name" required>
        <Input value={name} onChange={setName} placeholder="Company name" onKeyDown={e => e.key === "Enter" && handleSubmit()} />
      </FormField>
      <FormField label="ARR" required>
        <div style={{ position: "relative" }}>
          ...
          <input
            ...
            style={{
              width: "100%", background: C.bgCard, border: `1px solid ${C.borderDefault}`,
              borderRadius: 8, color: C.textPrimary, fontFamily: FONT_MONO, fontSize: 14,
              padding: "10px 12px 10px 28px", outline: "none",
          ...
          {riskOptions.map(level => (
            <button
              ...
              style={{
                ...
                border: `1px solid ${riskLevel === level ? riskColors[level] + "60" : C.borderDefault}`,
                background: riskLevel === level ? riskColors[level] + "14" : "transparent",
                color: riskLevel === level ? riskColors[level] : C.textSecondary,
                fontFamily: FONT_SANS, fontSize: 13, fontWeight: riskLevel === level ? 600 : 500,
      ...
      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 8 }}>
        <Btn variant="ghost" onClick={onClose}>Cancel</Btn>
        <Btn variant="primary" onClick={handleSubmit} disabled={!name.trim() || !arr || !renewalDate}>Add Account</Btn>
      </div>
    </Modal>
  );
}
...
function RenewalsGuidance({ accounts, onNavigate }) {
  const GUIDANCE_CACHE_KEY = `bc2-${store._ws}-renewals-guidance`;
  const [briefing, setBriefing] = useState(() => safeParse(localStorage.getItem(GUIDANCE_CACHE_KEY), null));
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const cachedAgo = briefing?._generatedAt
    ? (() => { const m = Math.floor((Date.now() - briefing._generatedAt) / 60000); return m < 1 ? "just now" : m < 60 ? `${m}m ago` : m < 1440 ? `${Math.floor(m/60)}h ago` : `${Math.floor(m/1440)}d ago`; })()
    : null;
  ...
  async function generateBriefing() {
    ...
    try {
      const portfolioData = accounts.map(a => {
        ...
        const contextCount = renewalStore.getContext(a.id).length;
        const threadCount = renewalStore.getThreads(a.id).length;
        return {
          name: a.name, arr: a.arr, renewalDate: a.renewalDate, riskLevel: a.riskLevel,
          daysUntilRenewal: daysUntil, contactCount: (a.contacts || []).length,
          contextItems: contextCount, activeThreads: threadCount,
...
PORTFOLIO SUMMARY: ${accounts.length} accounts, ${formatCurrency(totalARR)} total ARR, ${formatCurrency(atRiskARR)} at-risk ARR, ${due30} due in 30 days, ${due60} due in 60 days, ${due90} due in 90 days.

Generate a prioritized daily briefing. Return ONLY valid JSON (no markdown fences):
{
  "summary": "2-3 sentence overview of the portfolio posture today. Be direct and specific — reference account names and dollar amounts. What's the headline?",
  "priorities": [
    {
      "accountName": "Account name",
      "urgency": "critical|high|medium|opportunity",
      "reasoning": "WHY this needs attention today — be specific about the data signals. 1-2 sentences.",
      "action": "WHAT the specialist should do — specific next step, not generic advice.",
      "daysUntilRenewal": 30,
      "arr": 187000
    }
  ],
  "onTrack": ["Account names that are on-track and don't need immediate attention"],
  "pendingDrafts": 0
}

RULES:
- "priorities" should include every account that needs attention, ordered by urgency (critical first).
- Urgency levels: critical = overdue or ≤14 days + high risk; high = ≤30 days or high risk; medium = 31-60 days or medium risk with signals; opportunity = low risk with upsell/expansion potential.
- "onTrack" accounts get a brief mention, not full priority cards.
- Be specific — reference actual account names, ARR, days to renewal. Never be generic.
- If an account has 0 context items and 0 threads, flag the data gap in reasoning.
- "pendingDrafts" is always 0 for now (outbox feature coming later).`;
      ...
      parsed._generatedAt = Date.now();
      setBriefing(parsed);
      localStorage.setItem(GUIDANCE_CACHE_KEY, JSON.stringify(parsed));
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
  ...
  // Auto-generate on first load if accounts exist but no cached briefing
  useEffect(() => {
    if (accounts.length > 0 && !briefing && !loading) generateBriefing();
  }, [accounts.length]);
  ...
  if (accounts.length === 0) {
    return (
      <div style={{
        background: C.bgCard, border: `1px solid ${C.borderDefault}`,
        borderRadius: 12, padding: 32, minHeight: 400,
      }}>
        <RenewalsPlaceholder
          icon={Compass}
          title="Guidance"
          agent="Orchestration Agent"
          description="Add accounts to get AI-generated prioritization briefings. The Orchestration Agent will tell you which accounts need attention and why."
        />
      </div>
    );
  }
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
        {[
          { label: "Total ARR", value: formatCurrency(totalARR), color: C.textPrimary },
          { label: "At-Risk ARR", value: formatCurrency(atRiskARR), color: atRiskARR > 0 ? C.red : C.textTertiary },
          { label: "Due 30 Days", value: due30, color: due30 > 0 ? C.red : C.green },
          { label: "Due 60 Days", value: due60, color: due60 > 0 ? C.amber : C.textTertiary },
          { label: "Due 90 Days", value: due90, color: due90 > 0 ? C.textSecondary : C.textTertiary },
        ].map((stat, i) => (
          <div key={i} style={{
      ...
      <div style={{
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14, position: "relative" }}>
          ...
          <button onClick={generateBriefing} disabled={loading} style={{
            background: loading ? C.aiBlueMuted : "rgba(255,255,255,0.06)", border: "none",
            borderRadius: 6, padding: "4px 10px", cursor: loading ? "wait" : "pointer",
            fontFamily: FONT_SANS, fontSize: 12, fontWeight: 500,
            color: loading ? C.aiBlue : C.textTertiary, transition: "all 0.15s",
            display: "flex", alignItems: "center", gap: 6,
          }}
            onMouseEnter={e => { if (!loading) e.currentTarget.style.color = C.textPrimary; }}
            onMouseLeave={e => { if (!loading) e.currentTarget.style.color = C.textTertiary; }}
          >
            <Sparkles size={11} style={{ animation: loading ? "aiPulse 2s ease-in-out infinite" : "none" }} />
            {loading ? "Analyzing..." : "Refresh"}
          </button>
        </div>

        {error && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, color: C.red, fontFamily: FONT_BODY, fontSize: 13, marginBottom: 12 }}>
            <AlertTriangle size={14} /> {error}
          </div>
        )}
        ...
        {loading && !briefing ? (
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: C.aiBlue, animation: "aiPulse 2s ease-in-out infinite" }} />
            <span style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary }}>Analyzing your {accounts.length} accounts...</span>
          </div>
        ) : briefing?.summary ? (
          <div style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textSecondary, lineHeight: 1.7, position: "relative" }}>
            {briefing.summary}
          </div>
        ) : (
          <div style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary }}>
            Click Refresh to generate your daily briefing.
          </div>
        )}

        {/* Pending drafts outbox teaser */}
        {briefing && (
          <div style={{ marginTop: 14, paddingTop: 12, borderTop: `1px solid ${C.borderDefault}`, display: "flex", alignItems: "center", gap: 8 }}>
            <FileText size={13} style={{ color: C.textTertiary }} />
            <span style={{ fontFamily: FONT_MONO, fontSize: 12, color: C.textTertiary }}>
              {briefing.pendingDrafts || 0} pending drafts
            </span>
          </div>
        )}
      </div>
      ...
      {briefing?.priorities && briefing.priorities.length > 0 && (
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 14 }}>
            <span style={{ fontFamily: FONT_SANS, fontSize: 16, fontWeight: 600, color: C.textPrimary }}>Today's Priorities</span>
            <div style={{ flex: 1, height: 1, background: C.borderDefault }} />
            <span style={{ fontFamily: FONT_MONO, fontSize: 12, color: C.textTertiary }}>{briefing.priorities.length} accounts need attention</span>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {briefing.priorities.map((item, i) => {
              const color = urgencyColors[item.urgency] || C.textTertiary;
              const matchedAccount = accounts.find(a => a.name === item.accountName);
              return (
                <button
                  key={i}
                  onClick={() => matchedAccount && onNavigate(matchedAccount.id)}
                  style={{
                    display: "flex", gap: 16, padding: "16px 20px",
                    background: C.bgCard, border: `1px solid ${C.borderDefault}`, borderLeft: `3px solid ${color}`,
                    borderRadius: 10, cursor: matchedAccount ? "pointer" : "default",
                ...
                >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6, flexShrink: 0, minWidth: 48 }}>
                    <span style={{
                      fontFamily: FONT_MONO, fontSize: 10, fontWeight: 700, color: color,
                      background: color + "18", padding: "2px 8px", borderRadius: 4,
                      letterSpacing: "0.06em",
                    }}>
                      {urgencyLabels[item.urgency] || item.urgency}
                    </span>
                    <span style={{ fontFamily: FONT_MONO, fontSize: 18, fontWeight: 700, color: C.textTertiary, opacity: 0.4 }}>
                      {i + 1}
                    </span>
                  </div>
                  {/* Content */}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
                      ...
                      <span style={{
                        fontFamily: FONT_MONO, fontSize: 11, marginLeft: "auto",
                        color: (item.daysUntilRenewal || 999) <= 30 ? C.red : (item.daysUntilRenewal || 999) <= 60 ? C.amber : C.textTertiary,
                      }}>
                        {item.daysUntilRenewal > 0 ? `${item.daysUntilRenewal}d` : item.daysUntilRenewal === 0 ? "Today" : `${Math.abs(item.daysUntilRenewal)}d overdue`}
                      </span>
                    </div>
                    <div style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.textSecondary, lineHeight: 1.5, marginBottom: 6 }}>
                      {item.reasoning}
                    </div>
                    <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                      <Zap size={12} style={{ color: C.gold }} />
                      <span style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.gold, fontWeight: 500 }}>{item.action}</span>
                    </div>
                  </div>
      ...
      {/* On-track accounts */}
      {briefing?.onTrack && briefing.onTrack.length > 0 && (
        <div style={{
          background: C.bgCard, border: `1px solid ${C.borderDefault}`, borderRadius: 10, padding: "16px 20px",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
            <Check size={14} style={{ color: C.green }} />
            <span style={{ fontFamily: FONT_SANS, fontSize: 14, fontWeight: 600, color: C.textSecondary }}>On Track</span>
          </div>
          <div style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.textTertiary, lineHeight: 1.6 }}>
            {briefing.onTrack.join(" · ")}
          </div>
        </div>
      )}

      {/* All accounts list for navigation */}
      <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
          <span style={{ fontFamily: FONT_SANS, fontSize: 14, fontWeight: 600, color: C.textSecondary }}>All Accounts ({accounts.length})</span>
          <div style={{ flex: 1, height: 1, background: C.borderDefault }} />
        </div>
        {[...accounts]
          .sort((a, b) => {
          ...
          .map(account => {
            const daysUntil = Math.ceil((new Date(account.renewalDate) - now) / 86400000);
            ...
            return (
              <button
                ...
                onMouseLeave={e => { e.currentTarget.style.borderColor = C.borderDefault; e.currentTarget.style.background = C.bgCard; }}
              >
                <div style={{ width: 8, height: 8, borderRadius: "50%", flexShrink: 0, background: riskColors[account.riskLevel] || C.textTertiary }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontFamily: FONT_SANS, fontSize: 14, fontWeight: 600, color: C.textPrimary }}>{account.name}</div>
                </div>
                <span style={{ fontFamily: FONT_MONO, fontSize: 13, color: C.textTertiary }}>{formatCurrency(account.arr)}</span>
                <span style={{
                  fontFamily: FONT_MONO, fontSize: 11,
                  color: daysUntil <= 30 ? C.red : daysUntil <= 60 ? C.amber : C.textTertiary,
                }}>
                  {daysUntil > 0 ? `${daysUntil}d` : daysUntil === 0 ? "Today" : `${Math.abs(daysUntil)}d overdue`}
                </span>
...
function RenewalsAccountsView({ accounts, selectedAccount, onSelectAccount, onAddAccount }) {
  ...
  const [pasteSource, setPasteSource] = useState("manual");
  ...
  // Load threads when account changes
  useEffect(() => {
    if (!selectedAccount) { setThreads([]); setActiveThreadId(null); setMessages([]); setContextItems([]); return; }
    const acctThreads = renewalStore.getThreads(selectedAccount.id);
    setThreads(acctThreads);
    if (acctThreads.length > 0) {
      const sorted = [...acctThreads].sort((a, b) => new Date(b.lastMessageAt || b.createdAt) - new Date(a.lastMessageAt || a.createdAt));
    ...
    setContextItems(renewalStore.getContext(selectedAccount.id));
  }, [selectedAccount?.id]);

  // Load messages when thread changes
  useEffect(() => {
    if (!activeThreadId) { setMessages([]); return; }
    setMessages(renewalStore.getMessages(activeThreadId));
  }, [activeThreadId]);

  // Auto-scroll
  useEffect(() => { messagesEndRef.current?.scrollIntoView({ behavior: "smooth" }); }, [messages]);

  // Focus input when account/thread changes
  useEffect(() => { if (selectedAccount && activeThreadId) inputRef.current?.focus(); }, [selectedAccount?.id, activeThreadId]);

  function refreshContext() {
    if (selectedAccount) setContextItems(renewalStore.getContext(selectedAccount.id));
  }
  ...
  function createThread(firstMessage) {
    ...
    renewalStore.addThread(selectedAccount.id, thread);
    setThreads(renewalStore.getThreads(selectedAccount.id));
  ...
  function deleteThread(threadId) {
    renewalStore.deleteThread(selectedAccount.id, threadId);
    const remaining = renewalStore.getThreads(selectedAccount.id);
    setThreads(remaining);
    if (activeThreadId === threadId) {
      if (remaining.length > 0) {
        const sorted = [...remaining].sort((a, b) => new Date(b.lastMessageAt || b.createdAt) - new Date(a.lastMessageAt || a.createdAt));
        setActiveThreadId(sorted[0].id);
      } else {
        setActiveThreadId(null);
        setMessages([]);
  ...
  function deleteContextItem(itemId) {
    renewalStore.deleteContextItem(selectedAccount.id, itemId);
    refreshContext();
  }
  ...
  function handleCSVUpload(e) {
    ...
    reader.onload = (ev) => {
      ...
      const item = {
        ...
        source: "manual",
        content: text,
        metadata: { rows: rowCount, words: wordCount, size: (file.size / 1024).toFixed(1) + " KB" },
        uploadedAt: new Date().toISOString(),
      };
      renewalStore.addContextItem(selectedAccount.id, item);
      refreshContext();
    };
  ...
  function handleTextPasteSave() {
    ...
    const item = {
      ...
      metadata: { words: wordCount, size: (new Blob([pasteContent]).size / 1024).toFixed(1) + " KB" },
      uploadedAt: new Date().toISOString(),
    };
    renewalStore.addContextItem(selectedAccount.id, item);
    refreshContext();
    setPasteLabel("");
    setPasteSource("manual");
    setPasteContent("");
  ...
  function handleImageUpload(e) {
    ...
    reader.onload = (ev) => {
      ...
      const item = {
        ...
        source: "manual",
        content: base64,
        metadata: { size: (file.size / 1024).toFixed(1) + " KB", mimeType: file.type },
        uploadedAt: new Date().toISOString(),
      };
      renewalStore.addContextItem(selectedAccount.id, item);
      refreshContext();
  ...
  function buildCoPilotSystemPrompt() {
    ...
    const items = renewalStore.getContext(acct.id);
    const daysToRenewal = acct.renewalDate ? Math.ceil((new Date(acct.renewalDate) - new Date()) / 86400000) : null;

    let contextSummary;
    if (items.length === 0) {
      contextSummary = "No context items ingested yet.";
    } else {
      contextSummary = items.map(ci => {
        if (ci.type === "image") return `[IMAGE] ${ci.label || "Screenshot"}: (image attached — describe what you see if referenced)`;
        return `[${ci.type?.toUpperCase() || "NOTE"}] ${ci.label || "Untitled"}: ${ci.content?.slice(0, 800) || "(no content)"}`;
      }).join("\n\n");
    }

    return `You are a Renewal Co-Pilot inside Base Command. You assist a Renewal Specialist with their account work.

ACCOUNT CONTEXT:
- Name: ${acct.name}
- ARR: $${(acct.arr || 0).toLocaleString()}
- Renewal Date: ${acct.renewalDate || "Not set"}${daysToRenewal !== null ? ` (${daysToRenewal} days away)` : ""}
- Risk Level: ${acct.riskLevel || "unknown"}
- Created: ${acct.createdAt || "unknown"}

INGESTED DATA (${items.length} item${items.length !== 1 ? "s" : ""}):
${contextSummary}

Today: ${new Date().toLocaleDateString("en-US", { weekday: "long", month: "long", day: "numeric", year: "numeric" })}

CAPABILITIES:
- Email drafting (initial outreach, renewal options, expansion incentives, churn mitigation)
- Conversation prep and talk tracks
- Risk analysis and mitigation strategies
- Pricing strategies and discount frameworks
- Next best action recommendations
- Contract summary and term analysis
- Stakeholder mapping and engagement plans

GUIDELINES:
- Be direct, specific, and actionable. You're talking to a Renewal Specialist who knows their accounts.
- Reference the account data and ingested context above when relevant. Don't ask for info you already have.
- When drafting emails, match a professional but warm tone. Include subject lines.
- For risk analysis, be honest about signals and concrete about mitigations.
- Format with markdown. Use **bold** for key points. Keep responses focused and scannable.
- If asked about something outside the account context, help think it through using renewal expertise.`;
  }
  ...
  async function handleSend() {
    ...
    const userMsg = { role: "user", content: text, timestamp: new Date().toISOString() };
    renewalStore.addMessage(threadId, userMsg);
    const currentMessages = [...messages, userMsg];
    setMessages(currentMessages);

    const updatedThreads = renewalStore.getThreads(selectedAccount.id).map(t => {
      if (t.id !== threadId) return t;
      const isFirst = currentMessages.filter(m => m.role === "user").length === 1;
      return {
        ...t,
        ...(isFirst ? { title: text.slice(0, 50) + (text.length > 50 ? "..." : "") } : {}),
        lastMessageAt: new Date().toISOString(),
      };
    });
    renewalStore.saveThreads(selectedAccount.id, updatedThreads);
    ...
    try {
      ...
      const assistantMsg = { role: "assistant", content: response.toString(), timestamp: new Date().toISOString() };
      renewalStore.addMessage(threadId, assistantMsg);
      setMessages(prev => [...prev, assistantMsg]);
    } catch (err) {
      const errorMsg = { role: "assistant", content: `**Error:** ${err.message}`, timestamp: new Date().toISOString(), isError: true };
      renewalStore.addMessage(threadId, errorMsg);
      setMessages(prev => [...prev, errorMsg]);
    }
    setSending(false);
  }

  // ── Context panel type groups ──
  const contextByType = { csv: [], text: [], image: [] };
  contextItems.forEach(ci => {
    if (contextByType[ci.type]) contextByType[ci.type].push(ci);
    else contextByType.text.push(ci);
  });
  const typeConfig = {
    csv: { icon: FileText, label: "CSV Data", color: C.green },
    text: { icon: Type, label: "Text / Notes", color: C.amber },
    image: { icon: Image, label: "Screenshots", color: C.aiBlue },
  };
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          <MessageSquare size={28} style={{ color: C.gold }} />
        </div>
        <div>
          <h2 style={{ fontFamily: FONT_SANS, fontSize: 20, fontWeight: 600, color: C.textPrimary, margin: "0 0 4px", textAlign: "center" }}>No Accounts Yet</h2>
          <p style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary, maxWidth: 360, lineHeight: 1.5, margin: "8px 0 0", textAlign: "center" }}>
            Add your first account to start working with the Account Intelligence co-pilot.
          </p>
        </div>
        <Btn variant="primary" onClick={onAddAccount}>
          <Plus size={14} style={{ marginRight: 4 }} /> Add Account
        </Btn>
      </div>
    );
  ...
  return (
    <div style={{ display: "flex", gap: 0, minHeight: 500, height: "calc(100vh - 280px)" }}>
      ...
      <div style={{
        width: 240, flexShrink: 0, background: C.bgCard, border: `1px solid ${C.borderDefault}`,
        borderRadius: "12px 0 0 12px", overflow: "hidden", display: "flex", flexDirection: "column",
      }}>
        <div style={{
          padding: "14px 16px", borderBottom: `1px solid ${C.borderDefault}`,
          fontFamily: FONT_SANS, fontSize: 13, fontWeight: 600, color: C.textSecondary,
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: "none", borderLeft: `3px solid ${selected ? C.gold : "transparent"}`,
                  transition: "all 0.12s",
                }}
              ...
              >
                <div style={{
                  ...
                  background: riskColors[account.riskLevel] || C.textTertiary,
                }} />
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{
                    fontFamily: FONT_SANS, fontSize: 14, fontWeight: selected ? 600 : 500,
                    color: selected ? C.textPrimary : C.textSecondary,
                    overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                  }}>{account.name}</div>
                  <div style={{ fontFamily: FONT_MONO, fontSize: 11, color: C.textTertiary, marginTop: 2 }}>
      ...
      <div style={{
        flex: 1, background: C.bgCard, borderTop: `1px solid ${C.borderDefault}`,
        borderBottom: `1px solid ${C.borderDefault}`,
        ...(showContext ? {} : { borderRight: `1px solid ${C.borderDefault}`, borderRadius: "0 12px 12px 0" }),
        ...
        {selectedAccount ? (
          <>
            {/* Account header bar */}
            <div style={{
              padding: "12px 20px", borderBottom: `1px solid ${C.borderDefault}`,
              display: "flex", alignItems: "center", gap: 12, flexShrink: 0,
            }}>
              <h2 style={{ fontFamily: FONT_SANS, fontSize: 17, fontWeight: 600, color: C.textPrimary, margin: 0, flex: 1 }}>
                {selectedAccount.name}
              </h2>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                {[
                  { label: `$${(selectedAccount.arr || 0).toLocaleString()}`, color: C.textPrimary },
                  { label: selectedAccount.renewalDate || "No date", color: C.textSecondary },
                  { label: selectedAccount.riskLevel, color: { high: C.red, medium: C.amber, low: C.green }[selectedAccount.riskLevel] || C.textTertiary },
                ].map((chip, i) => (
                  <span key={i} style={{
                    fontFamily: FONT_MONO, fontSize: 11, fontWeight: 500, color: chip.color,
                    background: C.bgPrimary, padding: "3px 10px", borderRadius: 4, border: `1px solid ${C.borderDefault}`,
                    textTransform: "capitalize",
                  }}>{chip.label}</span>
                ))}
                <span style={{
                  fontFamily: FONT_MONO, fontSize: 11, fontWeight: 500, color: C.aiBlue,
                  background: C.aiBlueMuted, padding: "3px 10px", borderRadius: 4,
                }}>Co-Pilot</span>
                <button
                  ...
                  style={{
                    ...
                    background: showContext ? "rgba(255,255,255,0.07)" : "transparent",
                    border: `1px solid ${showContext ? C.aiBlue + "40" : C.borderDefault}`, borderRadius: 4,
                    cursor: "pointer", fontFamily: FONT_MONO, fontSize: 11, fontWeight: 500,
                    color: showContext ? C.aiBlue : C.textTertiary, transition: "all 0.12s",
                  }}
                  onMouseEnter={e => { if (!showContext) e.currentTarget.style.color = C.textSecondary; }}
                  onMouseLeave={e => { if (!showContext) e.currentTarget.style.color = C.textTertiary; }}
                >
                  <Eye size={12} /> Context{contextItems.length > 0 ? ` (${contextItems.length})` : ""}
                </button>
              </div>
            </div>
            ...
            {/* Thread bar */}
            <div style={{
              padding: "8px 20px", borderBottom: `1px solid ${C.borderDefault}`,
              display: "flex", alignItems: "center", gap: 8, flexShrink: 0, background: C.bgPrimary,
            }}>
              <button
                onClick={() => setShowThreadList(!showThreadList)}
                style={{
                  display: "flex", alignItems: "center", gap: 6, padding: "4px 10px",
                  background: showThreadList ? "rgba(255,255,255,0.07)" : "transparent",
                  border: `1px solid ${C.borderDefault}`, borderRadius: 6, cursor: "pointer",
                  fontFamily: FONT_SANS, fontSize: 12, fontWeight: 500, color: C.textSecondary,
                  transition: "all 0.12s",
                }}
                onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
                onMouseLeave={e => { if (!showThreadList) e.currentTarget.style.background = "transparent"; }}
              >
                <MessageSquare size={13} />
                {activeThreadId ? (threads.find(t => t.id === activeThreadId)?.title || "Conversation") : "No conversation"}
                <ChevronDown size={12} style={{ transform: showThreadList ? "rotate(180deg)" : "none", transition: "transform 0.15s" }} />
              </button>
              <div style={{ flex: 1 }} />
              <button
                ...
                style={{
                  ...
                  cursor: "pointer", fontFamily: FONT_SANS, fontSize: 12, fontWeight: 500, color: C.textSecondary,
                  transition: "all 0.12s",
                }}
                onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,0.05)"}
                onMouseLeave={e => e.currentTarget.style.background = "transparent"}
            ...
            {showThreadList && threads.length > 0 && (
              <div style={{
                borderBottom: `1px solid ${C.borderDefault}`, background: C.bgPrimary,
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        borderLeft: `2px solid ${isActive ? C.aiBlue : "transparent"}`,
                      }}
                      onClick={() => { setActiveThreadId(thread.id); setShowThreadList(false); }}
                      onMouseEnter={e => { if (!isActive) e.currentTarget.style.background = "rgba(255,255,255,0.03)"; }}
                      onMouseLeave={e => { if (!isActive) e.currentTarget.style.background = "transparent"; }}
                    >
                      <MessageSquare size={12} style={{ color: isActive ? C.aiBlue : C.textTertiary, flexShrink: 0 }} />
                      <span style={{
                        fontFamily: FONT_BODY, fontSize: 13, color: isActive ? C.textPrimary : C.textSecondary,
                        fontWeight: isActive ? 500 : 400, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                      }}>{thread.title}</span>
                      <span style={{ fontFamily: FONT_MONO, fontSize: 10, color: C.textTertiary }}>
                        {new Date(thread.lastMessageAt || thread.createdAt).toLocaleDateString([], { month: "short", day: "numeric" })}
                      </span>
                      <button
                        onClick={e => { e.stopPropagation(); deleteThread(thread.id); }}
                        style={{
                          background: "transparent", border: "none", cursor: "pointer", padding: 2,
                          color: C.textTertiary, display: "flex", alignItems: "center", opacity: 0.5,
            ...
            <div style={{ flex: 1, overflow: "auto", padding: "16px 20px", display: "flex", flexDirection: "column", gap: 16 }}>
              {!activeThreadId && messages.length === 0 && (
                <div style={{
                  display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                  flex: 1, gap: 16, padding: "40px 0", textAlign: "center",
                }}>
                  <div style={{
                  ...
                  <div>
                    <h3 style={{ fontFamily: FONT_SANS, fontSize: 16, fontWeight: 600, color: C.textPrimary, margin: "0 0 6px" }}>
                      Account Co-Pilot
                    </h3>
                    <p style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.textTertiary, maxWidth: 380, lineHeight: 1.5, margin: 0 }}>
                      Start a conversation about {selectedAccount.name}. Get help with renewal strategy, email drafts, risk analysis, and next best actions.
                    </p>
                  </div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap", justifyContent: "center", marginTop: 4 }}>
                    {[
                      "Draft a renewal outreach email",
                      "Analyze risk signals for this account",
                      "What's my best expansion play here?",
                    ].map((suggestion, i) => (
                      <button
                        key={i}
                        onClick={() => { setInput(suggestion); inputRef.current?.focus(); }}
                        style={{
                          ...
                          background: "transparent", border: `1px solid ${C.borderDefault}`,
                          color: C.textSecondary, fontFamily: FONT_BODY, fontSize: 12,
                          transition: "all 0.15s",
                        }}
                        onMouseEnter={e => { e.currentTarget.style.borderColor = C.borderSubtle; e.currentTarget.style.color = C.textPrimary; }}
                        onMouseLeave={e => { e.currentTarget.style.borderColor = C.borderDefault; e.currentTarget.style.color = C.textSecondary; }}
              ...
              {activeThreadId && messages.length === 0 && (
                <div style={{
                  display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center",
                  flex: 1, gap: 12, padding: "40px 0", textAlign: "center",
                }}>
                  <Sparkles size={20} style={{ color: C.aiBlue, opacity: 0.5 }} />
                  <p style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.textTertiary }}>
                    New conversation — ask anything about {selectedAccount.name}
                  </p>
                </div>
              )}
              ...
              {messages.map((msg, i) => (
                <div key={i} style={{
                  ...
                  <div style={{
                    ...
                    {msg.role === "user"
                      ? <span style={{ fontFamily: FONT_SANS, fontSize: 11, fontWeight: 700, color: C.gold }}>M</span>
                  ...
                  <div style={{
                    ...
                    <div style={{
                      fontFamily: FONT_BODY, fontSize: 13, color: C.textPrimary, lineHeight: 1.6,
                      whiteSpace: "pre-wrap", wordBreak: "break-word",
                    }}>
                      {msg.content}
                    </div>
                    <div style={{
                      fontFamily: FONT_MONO, fontSize: 10, color: C.textTertiary, marginTop: 5,
                      textAlign: msg.role === "user" ? "right" : "left",
                    }}>
                      {new Date(msg.timestamp).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                    </div>
              ...
              {sending && (
                <div style={{ display: "flex", gap: 10 }}>
                  ...
                  <div style={{
                    ...
                    <div style={{ fontFamily: FONT_BODY, fontSize: 13, color: C.textTertiary, animation: "pulse 1.5s ease-in-out infinite" }}>
                      Thinking...
                    </div>
                  </div>
                </div>
            ...
            {/* Chat input */}
            <div style={{
              padding: "12px 20px", borderTop: `1px solid ${C.borderDefault}`, flexShrink: 0,
              ...
              <div style={{ display: "flex", gap: 8 }}>
                <input
                  ...
                  onChange={e => setInput(e.target.value)}
                  onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSend(); } }}
                  placeholder={`Ask about ${selectedAccount.name}...`}
                  disabled={sending}
                  style={{
                  ...
                  onFocus={e => e.target.style.borderColor = C.aiBlue}
                ...
                <button
                  ...
                  style={{
                    padding: "10px 16px", borderRadius: 8, border: "none", cursor: sending || !input.trim() ? "default" : "pointer",
                    background: sending || !input.trim() ? C.bgElevated : `linear-gradient(135deg, ${C.gold}, ${C.aiBlue})`,
                    color: sending || !input.trim() ? C.textTertiary : "#fff",
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", flex: 1, gap: 12 }}>
            <MessageSquare size={32} style={{ color: C.textTertiary, opacity: 0.5 }} />
            <p style={{ fontFamily: FONT_BODY, fontSize: 14, color: C.textTertiary }}>Select an account to start working</p>
          </div>
        )}
      </div>
      ...
      {showContext && selectedAccount && (
        <div style={{
          ...
          border: `1px solid ${C.borderDefault}`, borderLeft: "none",
          borderRadius: "0 12px 12px 0", display: "flex", flexDirection: "column", overflow: "hidden",
        }}>
          {/* Context panel header */}
          <div style={{
            padding: "12px 16px", borderBottom: `1px solid ${C.borderDefault}`,
            display: "flex", alignItems: "center", gap: 8,
          }}>
            <Eye size={14} style={{ color: C.aiBlue }} />
            <span style={{ fontFamily: FONT_SANS, fontSize: 13, fontWeight: 600, color: C.textPrimary, flex: 1 }}>Account Context</span>
            <button
              onClick={() => setShowContext(false)}
              style={{ background: "transparent", border: "none", cursor: "pointer", color: C.textTertiary, display: "flex", padding: 2 }}
            >
              <X size={14} />
            </button>
          </div>
          ...
          <div style={{ flex: 1, overflow: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 16 }}>
            {/* Account summary */}
            <div style={{
              background: C.bgPrimary, borderRadius: 8, padding: 12,
              border: `1px solid ${C.borderDefault}`,
            }}>
              <div style={{ fontFamily: FONT_SANS, fontSize: 12, fontWeight: 600, color: C.textSecondary, marginBottom: 8 }}>
                Summary
              </div>
              {(() => {
                const dtr = selectedAccount.renewalDate ? Math.ceil((new Date(selectedAccount.renewalDate) - new Date()) / 86400000) : null;
                return (
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {[
                      { label: "ARR", value: `$${(selectedAccount.arr || 0).toLocaleString()}` },
                      { label: "Renewal", value: selectedAccount.renewalDate || "Not set" },
                      ...(dtr !== null ? [{ label: "Days Out", value: `${dtr} days`, color: dtr <= 30 ? C.red : dtr <= 60 ? C.amber : C.textPrimary }] : []),
                      { label: "Risk", value: selectedAccount.riskLevel || "unknown", color: { high: C.red, medium: C.amber, low: C.green }[selectedAccount.riskLevel] },
                      { label: "Threads", value: String(threads.length) },
                      { label: "Context", value: `${contextItems.length} item${contextItems.length !== 1 ? "s" : ""}` },
                    ].map((row, i) => (
                      <div key={i} style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <span style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.textTertiary }}>{row.label}</span>
                        <span style={{ fontFamily: FONT_MONO, fontSize: 12, fontWeight: 500, color: row.color || C.textPrimary, textTransform: "capitalize" }}>{row.value}</span>
                      </div>
                    ))}
                  </div>
                );
            ...
            <div>
              ...
              <div style={{ display: "flex", gap: 6 }}>
                ...
                {[
                  { label: "CSV", icon: FileText, color: C.green, onClick: () => csvInputRef.current?.click() },
                  { label: "Text", icon: Type, color: C.amber, onClick: () => setShowTextPaste(true) },
                  { label: "Image", icon: Image, color: C.aiBlue, onClick: () => imgInputRef.current?.click() },
                ].map((btn, i) => (
                  <button
                    key={i}
                    onClick={btn.onClick}
                    style={{
                      ...
                      background: "transparent", border: `1px solid ${C.borderDefault}`,
                      color: C.textSecondary, fontFamily: FONT_SANS, fontSize: 11, fontWeight: 500,
                      transition: "all 0.12s",
                    }}
            ...
            {showTextPaste && (
              <div style={{
                ...
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
                  <span style={{ fontFamily: FONT_SANS, fontSize: 12, fontWeight: 600, color: C.amber }}>Paste Text</span>
                  <button onClick={() => setShowTextPaste(false)} style={{ background: "transparent", border: "none", cursor: "pointer", color: C.textTertiary, display: "flex", padding: 2 }}>
                    <X size={12} />
                  </button>
                </div>
                <input
                  ...
                  placeholder="Label (e.g. Gong call notes)"
                  style={{
                    width: "100%", padding: "6px 10px", borderRadius: 6, marginBottom: 6,
                    background: C.bgCard, border: `1px solid ${C.borderDefault}`,
                    color: C.textPrimary, fontFamily: FONT_BODY, fontSize: 12, outline: "none",
                    boxSizing: "border-box",
                  }}
                />
                <select
                  ...
                  style={{
                    width: "100%", padding: "6px 10px", borderRadius: 6, marginBottom: 6,
                    background: C.bgCard, border: `1px solid ${C.borderDefault}`,
                    color: C.textSecondary, fontFamily: FONT_BODY, fontSize: 12, outline: "none",
                    boxSizing: "border-box",
                  }}
                >
                ...
                <textarea
                  ...
                  rows={5}
                  style={{
                    width: "100%", padding: "6px 10px", borderRadius: 6, marginBottom: 8, resize: "vertical",
                    background: C.bgCard, border: `1px solid ${C.borderDefault}`,
                    color: C.textPrimary, fontFamily: FONT_BODY, fontSize: 12, outline: "none",
                ...
                />
                <button
                  onClick={handleTextPasteSave}
                  disabled={!pasteContent.trim()}
                  style={{
                    width: "100%", padding: "7px 0", borderRadius: 6, border: "none", cursor: pasteContent.trim() ? "pointer" : "default",
                    background: pasteContent.trim() ? C.amber : C.bgElevated,
                    color: pasteContent.trim() ? "#000" : C.textTertiary,
                    fontFamily: FONT_SANS, fontSize: 12, fontWeight: 600,
                    transition: "all 0.12s", opacity: pasteContent.trim() ? 1 : 0.5,
                  }}
                >
                  Save Context
                </button>
            ...
            {/* Context items grouped by type */}
            {contextItems.length === 0 && !showTextPaste && (
              <div style={{
                textAlign: "center", padding: "20px 0",
              }}>
                <Upload size={20} style={{ color: C.textTertiary, opacity: 0.4, marginBottom: 8 }} />
                <p style={{ fontFamily: FONT_BODY, fontSize: 12, color: C.textTertiary, margin: 0, lineHeight: 1.5 }}>
                  No context loaded yet. Upload CSV data, paste text notes, or add screenshots to feed the Co-Pilot.
                </p>
              </div>
            )}

            {["csv", "text", "image"].map(type => {
              const items = contextByType[type];
              ...
              return (
                <div key={type}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 6 }}>
                    ...
                    <span style={{ fontFamily: FONT_SANS, fontSize: 12, fontWeight: 600, color: cfg.color }}>
                      {cfg.label}
                    </span>
                    <span style={{ fontFamily: FONT_MONO, fontSize: 10, color: C.textTertiary }}>({items.length})</span>
                  </div>
                  <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                    {items.map(item => (
                      <div
                        ...
                        style={{
                          ...
                          background: C.bgPrimary, borderRadius: 6, border: `1px solid ${C.borderDefault}`,
                        }}
                      >
                        {type === "image" && item.content && (
                          <img
                            src={item.content}
                            alt={item.label}
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{
                            fontFamily: FONT_BODY, fontSize: 12, color: C.textPrimary, fontWeight: 500,
                            overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",
                          }}>
                            {item.label}
                          </div>
                          <div style={{ fontFamily: FONT_MONO, fontSize: 10, color: C.textTertiary }}>
                            {item.source !== "manual" && <span style={{ textTransform: "capitalize" }}>{item.source} · </span>}
                            {item.metadata?.rows != null && `${item.metadata.rows} rows · `}
                            {item.metadata?.words != null && `${item.metadata.words} words · `}
                            {item.metadata?.size || ""}
                          </div>
                        </div>
                        <button
                          ...
                          style={{
                            background: "transparent", border: "none", cursor: "pointer", padding: 2,
                            color: C.textTertiary, display: "flex", alignItems: "center", opacity: 0.4,
                            transition: "opacity 0.1s",
                          }}
                          onMouseEnter={e => e.currentTarget.style.opacity = "1"}
...
function RenewalsPlaceholder({ icon: Icon, title, agent, description }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", minHeight: 320, gap: 16, textAlign: "center" }}>
      ...
      <div>
        <h2 style={{ fontFamily: FONT_SANS, fontSize: 20, fontWeight: 600, color: C.textPrimary, margin: "0 0 4px" }}>{title}</h2>
        <span style={{
          fontFamily: FONT_MONO, fontSize: 11, fontWeight: 500, color: C.aiBlue,
          background: C.aiBlueMuted, padding: "3px 10px", borderRadius: 4,
        }}>{agent}</span>
      ...
      <div style={{
        fontFamily: FONT_MONO, fontSize: 11, color: C.textTertiary,
        background: C.bgPrimary, padding: "6px 12px", borderRadius: 6,
        border: `1px solid ${C.borderDefault}`,
      }}>
        Coming up in build sequence
      </div>
    </div>
  );
}
      ...
      <main style={{ flex: 1, overflow: "auto", position: "relative" }}>
        ...
        <div style={{
          ...
          <div style={{ fontFamily: FONT_SANS, fontSize: 16, fontWeight: 600, color: C.textPrimary, letterSpacing: "-0.01em", textTransform: "capitalize" }}>
            {view === "dashboard" ? "Command Center" : view === "renewals" ? "Renewals" : view === "intel" ? "Intel" : view}
          </div>
        ...
        <div style={{ minHeight: "calc(100vh - 48px)" }}>
          ...
          {view === "intel" && (
            <IntelView setDecisions={setDecisions} setTasks={setTasks} setPriorities={setPriorities} setMeetings={setMeetings} ingestSessions={ingestSessions} setIngestSessions={setIngestSessions} focusSessionId={focusSessionId} setFocusSessionId={setFocusSessionId} decisions={decisions} tasks={tasks} priorities={priorities} projects={projects} />
          )}
          {view === "meetings" && (
